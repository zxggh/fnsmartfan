...
class STCController:
    ...
    async def send(self, cmd, wait=2.0):
        ...
        if not self._connected:
            return ""
        raw = await self._send_raw(cmd, wait=wait)
        if raw:
            return raw
        # 空响应 -> 重发一次(避免偶发丢失)
    ...
    async def _send_raw(self, cmd, wait=2.0, retries=0):
                    ...
                    while time.time() < deadline:
                        ...
                        try:
                            chunk = await asyncio.wait_for(
                                self._reader.read(2048), timeout=min(remaining, 0.5))
                        except asyncio.TimeoutError:
                            continue
                        except Exception:
    ...
    async def set_fan_speed(self, fan=1, speed=50):
        ...
        2秒无回复重发, 最多3次, 全部无反应则返回失败(不更新缓存).
        """
        speed = max(0, min(100, speed))
        for attempt in range(3):
            resp = await self.send(f"F{fan}CPD={speed}", wait=2.0)
            val = None
            m = re.search(rf"F{fan}_CPD=(\d+)%?", resp)
            if m:
                val = int(m.group(1))
            ...
            if val is not None:
                ...
                return {"ok": True, "value": val}
            if attempt < 2:
                self._log("SYS", f"F{fan}设置无回复, 重试{attempt+2}/3")
                await asyncio.sleep(0.3)
        self._log("SYS", f"F{fan}设置3次无响应, 忽略")
        return {"ok": False, "error": "3次无响应"}
